...
- **Claude Code / Big Pickle** - For development

## Workflow Steps

1. **Start with brand** - We know the brand (e.g., "ROPARDO")
...
3. **Scrape jobs** - Extract jobs from ROPARDO careers (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape ROPARDO API (jobs for Romania)
    │
    ▼
...
| `tests/validate-ropardo-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real ROPARDO API |
...
| Setting | Value | Where |
|---------|-------|-------|
| Delay between requests | 1000 ms | `index.js` — `sleep(1000)` |
