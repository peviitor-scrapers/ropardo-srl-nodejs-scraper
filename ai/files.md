...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-ropardo-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for ROPARDO used in unit tests |
| `tests/validate-ropardo-jobs.js` | **ROPARDO-specific fast validator (used by CI).** HEAD requests only, hardcoded ROPARDO CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ROPARDO portal, ANAF, and SOLR |
...
| ~~`AI-DERIVATION-GUIDE.md`~~ | ~~Deleted — belongs only in the EPAM template.~~ |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 5415866. Use only when you need to wipe ROPARDO jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape ROPARDO → transform → upsert SOLR → generate docs/jobs.md