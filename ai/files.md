...
| `scraper/anaf.js` | Multi-source company data module - ANAF + CUIFirma (MCP fallback + search). Exports `getCompanyFromANAF`, `getCompanyFromANAFWithFallback`, `searchCompany` |
...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-ropardo-jobs.js`, and the deep-validate workflow. |
...
| `scraper/config/scraper.json` | Scraper-specific config (apiBase, careersUrl) loaded via `scraper/config/scraper.js` |
...
| `tests/validate-ropardo-jobs.js` | **ROPARDO-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/unit/demoanaf.test.js` | Unit tests for anaf.js - search, company retrieval, CUIFirma fallback |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ROPARDO portal, ANAF, and Peviitor API |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for ROPARDO |
...
| `.github/workflows/job-seeker-ro-spider.yml` | Daily scraping workflow |
...
| `.github/workflows/job-recovery-from-disaster.yml` | Manual recovery workflow (re-upload jobs from docs/jobs.md) |
| `.github/workflows/automation-template-sync-check.yml` | Weekly check that derived scrapers are up to date with the template |
...
- Full workflow: validate company (ANAF+CUIFirma+Peviitor) → scrape ROPARDO → transform → upsert → generate docs/jobs.md
