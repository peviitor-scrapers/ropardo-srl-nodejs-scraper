...
async function fetchJobListings() {
  console.log(`Fetching ROPARDO jobs page: ${TARGET_URL}`);
...
function parseJobsHTML(html) {
  ...
  $(".list-item").each((_, el) => {
    ...
    const locationEl = $el.find(".meta-location").first();
    const location = locationEl.clone().children().remove().end().text().trim().replace(/,?\s*România$/i, '').trim();

    const scheduleEl = $el.find(".meta-shedule-duration").first();
    const schedule = scheduleEl.text().trim();

    const skillsEl = $el.find("p").filter((_, p) => $(p).text().includes("Tech skills:")).first();
    const skillsText = skillsEl.text().replace("Tech skills:", "").trim();
    const tags = skillsText ? skillsText.split(",").map(s => s.trim().toLowerCase()).filter(Boolean) : [];

    const urlEl = $el.find("a.button.job-details").first();
    const url = urlEl.attr("href") || "";

    let workmode = "on-site";
    if (schedule.toLowerCase().includes("remote")) workmode = "remote";
    else if (schedule.toLowerCase().includes("hybrid") || schedule.toLowerCase().includes("part time")) workmode = "hybrid";

    jobs.push({
      url,
      title,
      workmode,
      location: location ? [location] : ["Sibiu"],
      tags
    });
  });
...
async function scrapeAllListings() {
  console.log("Fetching job listings from ROPARDO careers page...");
...
async function main() {
  ...
  try {
    ...
    console.log("(Keeping existing jobs - will upsert ROPARDO jobs only)");
